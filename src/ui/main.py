...
class CandlestickChart:
    def __init__(self, parent, controller):
        ...
        self.canvas = tkinter.Canvas(self.parent, width=800, height=600)
        self.canvas.pack()
...
class Home(tkinter.Tk):
    def __init__(self, parent, controller):
        ...
        candlestick_menu = tkinter.Menu(menu_bar, tearoff=0)
        menu_bar.add_cascade(label='Candlestick Chart', menu=candlestick_menu)
        candlestick_menu.add_command(label='Refresh', command='')
        candlestick_menu.add_separator()
        file_menu.add_command(label='Logout', command=self.logout)
        file_menu.add_separator()
        file_menu.add_command(label='Exit', command=self.parent.destroy)
        self.protocol("WM_DELETE_WINDOW", self.parent.destroy)
        self.title('ZONES EA            |   Home')
